...
bool ModelDatabase::addModel(vtkPolyData *highResModel, UserData *userData, int numberOfPointsPerLayer,
                             double &relNumOfPairsInHashTable) {
    if (highResModel->GetPoints() == nullptr) {
        fprintf(stderr, "WARNING in 'ModelDatabase::%s()': the model is empty! It won't be inserted.\n", __func__);
        fflush(stderr);
        return false;
    }
...
double ModelDatabase::removePairsByKillingCellEntries(vector<HashTableCellEntry *> &cellEntries, int numOfPairs,
                                                      double toRemove) {
