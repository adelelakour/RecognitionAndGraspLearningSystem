...
public:
	...
	inline const double_2* getSafePixel(const double* p, int& x, int& y) const;
	inline void getPixelBounds(double* bounds, int x, int y);
	...
	double_2*** getPixels(){ return mPixels;}
	double getPixelSize(){ return mPixelSize;}
	const double* getBounds(){ return mBounds;}
...
protected:
	...
	double_2*** mPixels;
	double mPixelSize, mExtentX, mExtentY, mBounds[4];
...
inline const double_2* ORRRangeImage2::getSafePixel(const double* p, int& x, int& y) const
{
	x = (int)((p[0]-mBounds[0])/mPixelSize);
	y = (int)((p[1]-mBounds[2])/mPixelSize);
...
inline void ORRRangeImage2::getPixelBounds(double* bounds, int x, int y)
{
