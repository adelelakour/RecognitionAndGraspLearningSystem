...
inline void mat_mult3x3(double **m, double s)
{
	m[0][0] *= s; m[0][1] *= s; m[0][2] *= s;
	m[1][0] *= s; m[1][1] *= s; m[1][2] *= s;
	m[2][0] *= s; m[2][1] *= s; m[2][2] *= s;
...
inline void mat_mult3x3(const double **m, double s, double **out)
{
	out[0][0] = m[0][0]*s; out[0][1] = m[0][1]*s; out[0][2] = m[0][2]*s;
	out[1][0] = m[1][0]*s; out[1][1] = m[1][1]*s; out[1][2] = m[1][2]*s;
	out[2][0] = m[2][0]*s; out[2][1] = m[2][1]*s; out[2][2] = m[2][2]*s;
}
...
inline void mat_add3x3(double **res, const double **m)
{
	res[0][0] += m[0][0]; res[0][1] += m[0][1]; res[0][2] += m[0][2];
	res[1][0] += m[1][0]; res[1][1] += m[1][1]; res[1][2] += m[1][2];
	res[2][0] += m[2][0]; res[2][1] += m[2][1]; res[2][2] += m[2][2];
...
/** \brief out = A*p + t. */
template<class T>
inline void mat_affine_transform3(const T A[3][3], const T *t, const T *p, T *out)
{
	// Rotate and translate
	out[0] = A[0][0]*p[0] + A[0][1]*p[1] + A[0][2]*p[2] + t[0];
	out[1] = A[1][0]*p[0] + A[1][1]*p[1] + A[1][2]*p[2] + t[1];
	out[2] = A[2][0]*p[0] + A[2][1]*p[1] + A[2][2]*p[2] + t[2];
}
...
/** out = sRp + T */
inline void mat_mult_by_similarity3(double s, const double R[3][3], const double* T, const double* p, double* out)
{
	// Rotate, scale and translate
	out[0] = s*(R[0][0]*p[0] + R[0][1]*p[1] + R[0][2]*p[2]) + T[0];
	out[1] = s*(R[1][0]*p[0] + R[1][1]*p[1] + R[1][2]*p[2]) + T[1];
	out[2] = s*(R[2][0]*p[0] + R[2][1]*p[1] + R[2][2]*p[2]) + T[2];
}
...
inline void mat_length_of_columns3x3(double m[3][3], double len[3])
{
	len[0] = sqrt(m[0][0]*m[0][0] + m[1][0]*m[1][0] + m[2][0]*m[2][0]);
	len[1] = sqrt(m[0][1]*m[0][1] + m[1][1]*m[1][1] + m[2][1]*m[2][1]);
	len[2] = sqrt(m[0][2]*m[0][2] + m[1][2]*m[1][2] + m[2][2]*m[2][2]);
}
...
inline void mat_change_row_sign3x3(double M[3][3], const int row)
{
	M[row][0] = -M[row][0];
	M[row][1] = -M[row][1];
	M[row][2] = -M[row][2];
...
inline void mat_change_col_sign3x3(double M[3][3], const int col)
{
	M[0][col] = -M[0][col];
	M[1][col] = -M[1][col];
	M[2][col] = -M[2][col];
